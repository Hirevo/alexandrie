...
use crate::db::schema::*;
use crate::{AlexError, AppState, Auth, Crate, DbConn, Dependency, DependencyKind, Error, Indexer, Store};
...
pub fn api_publish(
    ...
    match state.index().max_version(krate.name.as_str()) {
        Ok(max_version) if metadata.vers <= max_version => Err(Error::from(
            AlexError::VersionTooLow(krate.name, max_version, metadata.vers),
        )),
        Ok(_) => {
